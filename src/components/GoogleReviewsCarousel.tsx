  ...
  if (variant === 'hero') {
    return (
      <>
         ...
         <div className="hidden md:block absolute right-2 md:right-4 lg:right-6 xl:right-8 top-[45%] md:top-1/2 -translate-y-1/2 z-20 w-[16rem] md:w-[20rem] lg:w-[24rem] xl:w-[28rem] 2xl:w-[32rem] max-w-[calc(100%-1rem)] md:max-w-[calc(100%-2rem)] lg:max-w-[calc(100%-3rem)] xl:max-w-[calc(100%-4rem)]">
           <div className="bg-white/30 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-3 md:p-4 lg:p-6 xl:p-8">
             <div className="flex items-center gap-2 mb-3 md:mb-4">
               ...
               <div>
                 <p className="font-bold text-xs md:text-sm text-secondary-950">Google Reviews</p>
                 <div className="flex items-center gap-1">
                   {[...Array(5)].map((_, i) => (
                     <Star key={i} className="w-2.5 h-2.5 md:w-3 md:h-3 fill-accent-400 text-accent-400" />
                   ))}
                   <span className="text-[10px] md:text-xs text-secondary-600 ml-1">5.0</span>
                 </div>
               </div>
             </div>
                  ...
                  >
                     <div className="flex items-center gap-1 mb-2">
                       {[...Array(review.rating)].map((_, i) => (
                         <Star key={i} className="w-3 h-3 md:w-4 md:h-4 fill-accent-400 text-accent-400" />
                       ))}
                     </div>
                     <p className="text-xs md:text-sm text-secondary-700 mb-2 md:mb-3 leading-relaxed line-clamp-3">
                       "{review.comment}"
                     </p>
                     <div className="flex items-center gap-1.5 md:gap-2">
                       <p className="font-semibold text-[10px] md:text-xs text-secondary-950">
                         {review.name}
                       </p>
                       <span className="text-secondary-400 text-[10px] md:text-xs">·</span>
                       <p className="text-[10px] md:text-xs text-secondary-600">{review.project}</p>
                     </div>
                  </div>
                ))}
              </div>
               ...
               {reviews.map((_, index) => (
                 <button
                   ...
                   className={`h-1 md:h-1.5 rounded-full transition-all duration-300 ${
                     index === currentIndex
                       ? 'w-5 md:w-6 bg-primary-500'
                       : 'w-1 md:w-1.5 bg-secondary-300 hover:bg-secondary-400'
                   }`}
